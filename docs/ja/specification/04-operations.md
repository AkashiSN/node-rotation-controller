...
| `noderotation_duration_seconds` | Histogram | `nodepool`, `phase` | phase ∈ {surge_wait, drain} ごとの所要時間。`surge_wait` = `started-at → surge_ready`、`drain` = `draining-at → 旧 NodeClaim の finalize`。`drain` は `pending → draining` 遷移時にスタンプされる NodePool の `draining-at` annotation をアンカーとする — 旧 NodeClaim の `deletionTimestamp` は、ヒストグラムを一度だけ観測する唯一の完了地点までに finalize されて失われているため、このプール側アンカーが必要（§5.3）。観測は**成功した遷移につき高々 1 回**：各サンプルはその phase 遷移を確定させる durable な annotation 書き込みの**後**でのみ取得されるため、書き込みがリトライされても（`resourceVersion` 競合や一時的な API エラーで同じ phase が再駆動されても）二重計上されない。これはヒストグラムにとって意図した trade である — 書き込みと観測の間にコントローラーが停止した場合、その 1 サンプルを取りこぼす（`_count`/`_sum` が正直に下がる）方を選び、どの rotation も要さなかった所要時間の phantom サンプルを注入しない。at-least-once 配置を保つ `noderotation_completed_total` カウンター（1 回の増分の欠落は重複より悪い）とは異なる |
...
| `no rotation candidate` | `reason` — ブロックしている開始ゲート（`outOfWindow`, `frozen`, `cooldownAfterSuccess`, `failurePause`）、または `noEligibleClaim` と内訳（`claims`, `notTriggered`, `inBackoff`, `inFlight`, `optedOut`, `deleting`, `notReady`, `terminal`）|
...
| `rotation complete` | `mode`, `drain`。surge 経路では加えて `surgeNode`, `surgeWait`, `total`（= `surgeWait` + `drain`）— ローテーション全体を自己完結した 1 行にまとめるので、先行パスで出力された `surge node ready` 行への join なしに end-to-end で監査できる。`surgeWait` は NodePool の `surge-wait` アンカー（§5.3）で持ち越され、`surgeNode` は unfreeze 前に surge ターゲットの `surge-for` マーカーから回収する。3 つとも surge フェーズを持たない surge-less な forceful-fallback 経路では存在しない |
...
失敗がこのコストを繰り返す速さは 2 つの機構が抑える: エスカレーションする `retryBackoff`（§5.3）が *同一* claim のリトライを抑え、**NodePool レベルの試行間休止**（`last-failure-at` + `failurePause`、§5.2 ステップ 2、ADR-0004）が候補の*巡回*を抑える — 後者がなければ、触れる claim をことごとく失敗させる系統的原因（執拗な placeholder preempt、持続的な同一 AZ 容量不足）は 1 分程度で次の候補へ移り、候補ごとに `readyTimeout` 分の失敗 surge 課金を焼き続けることになる。休止があれば、系統的失敗下の NodePool が走らせるのは最大でも **`readyTimeout + failurePause` あたり 1 試行**（既定値で約 25m。未設定の `failurePause` は `max(10m, cooldownAfter)` に解決される）であり、そのパターンには `noderotation_retry_count` がアラートする（§4.2）。`failurePause` は `cooldownAfter`（成功後の整定休止）とは別なので、ウィンドウのスループット目的で整定休止を下げてもこのコスト上限は弱まらない。
