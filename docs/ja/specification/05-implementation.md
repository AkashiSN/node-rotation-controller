...
- **条件付きクリアが発行者を確定する。** reap は呼び出し元が受け取った anchor から entry するが、それはキャッシュ読み取りであり、先行パスが既にクリアした anchor をなお指していることがある。したがって anchor をクリアする書き込みこそが、そのローテーションを reap したパスを識別する: reap されたローテーション 1 件につき Event はちょうど 1 件、それを実行したパスが、既に完了した作業を記述して発行する。これは §5.2 の完了パスが用いる claim-then-announce の順序であり、同じコストを受け入れる — 書き込みと発行の間でコントローラーが停止した場合、発行は失われる。
