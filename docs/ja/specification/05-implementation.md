...
advance(np, name):
  ...
  if cand == nil:                            # 旧 NodeClaim が finalize 消滅 → いずれにせよ終端。だがどちらの形で?
      ...
      clear(np, active-rotation, active-rotation-state, draining-at, surge-wait, rotation-mode)  # 同一オブジェクト → 1 回の update。ゲート解放は最後（surge-wait は surge ローテーション時のみ、rotation-mode は forceful fallback 時のみ存在、§5.3）
...
| `draining` | 旧 NodeClaim 消滅 | *(完了)* | unfreeze の**前**に `surge-for` マーカーから surge ノードを回収（完了行用）。placeholder 削除。`surge-for` を持つノードを unfreeze（+ マーカーで uncordon）。`last-rotation-at` 書込。success 計上 + `drain` フェーズ所要時間（`now − draining-at`、アンカーがある場合）。`rotation complete` を `total = surge-wait + drain` 付きでログ（§4.2）。アンカー + `active-rotation-state` + `draining-at` + `surge-wait` + `rotation-mode` clear（最後。`surge-wait` は surge ローテーション時のみ、`rotation-mode` は forceful fallback 時のみ存在）|
