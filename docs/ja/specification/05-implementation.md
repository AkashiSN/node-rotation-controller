...
flowchart TD
    subgraph cluster["Cluster（Karpenter v1+）"]
        subgraph ns["Namespace: node-rotation-system（設定可能）"]
            ctrl["node-rotation-controller（Deployment）<br/>controller-runtime manager、replicas=2 + leader election（active 1）<br/>NodePool reconciler; NodeClaim / Pod / Node + RotationPolicy を watch<br/>1 分の self-requeue（窓端・drain 進捗）、/metrics エンドポイント"]
        end
