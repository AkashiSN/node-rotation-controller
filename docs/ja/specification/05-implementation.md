...
- reconcile の残りの claim 状態書き込みは無条件のままだが、veto ではなく構造的に安全: `pending` → `draining`、forceful fallback の `draining`、`pending` → `failed` は、いずれもそれが記録する作業を直前に行ったパスが、自分の入口を既に主張したハンドラーの中で書き、いずれも claim を前へ進める。§5.3 の起動時 sweep の書き込みはこのディスパッチの外側にある
