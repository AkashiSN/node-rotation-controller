...
- **claim の状態書き込みはすべて同じ方法で遷移を主張する**。受け付けるのはそのハンドラーがディスパッチされる状態からのみ — `expired` へ入る 2 つの書き込み、`pending` の入口アサート、`failed` → `pending` のリトライ。終端状態が既に書かれた claim をキャッシュ経由で見たパスはクリーンアップと anchor 解放だけを行い何も発行しない。claim が別の状態へ進んでいたパスは一切書き込まず、ローテーションのランタイムオブジェクトにも触れず、それを所有するハンドラーに委ねる
