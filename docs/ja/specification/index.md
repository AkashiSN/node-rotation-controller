...
Karpenter 管理下のノードを、Karpenter の forceful な `expireAfter` が発火する前に、設定可能なメンテナンスウィンドウ内で make-before-break（surge）方式で先回りローテーションする Kubernetes コントローラの機能仕様書。
...
4. **[運用](./04-operations)** — [4.1 キャパシティ / 可用性](./04-operations#41-キャパシティ--可用性) · [4.2 観測性](./04-operations#42-観測性) · [4.3 RBAC とクラウド権限](./04-operations#43-rbac-とクラウド権限) · [4.4 コスト](./04-operations#44-コスト)
