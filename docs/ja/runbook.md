...
- `surge.drainEstimate` — **健全で PDB を尊重する drain が実際にかかる時間**。これが
  **スループットのつまみ**である。コントローラはこれから容量を予測する: 期待ローテーション
  サービス時間は `t_rot_est = readyTimeout + drainEstimate + buffer` であり、長さ `D` の
...
`drainEstimate` をワークロードの実際の drain 時間に設定する — それは
`noderotation_duration_seconds{phase="drain"}` から読める。スループット警告を消すために
...
`cooldownAfter` を下げる; `drainEstimate` が実 drain を過大評価しているなら訂正する。`tGP`
を下げても **効かない** — この述語は `t_rot_est` により評価され、そこにはもう `tGP` は
含まれない。
...
- **対処。** `noderotation_forceful_fallback_total` が増大し続けるのは graceful な surge がレースにたびたび負けることを意味する — メンテナンスウィンドウを拡大する; 予測が過大なら `surge.drainEstimate` をワークロードの実際の drain 時間に設定する; `ThroughputBurstShortfall` でフラグされた同期ノード数を削減する。（`tGP` は `A` を伸ばすために下げてもよいが、スループットのレバーではない。）
...
| **毎ウィンドウ** `ThroughputBurstShortfall` 警告 | `ThroughputBurstShortfall` Warning イベント | `K · C` がプールのノード台数を下回る — 共有 deadline までに同期バッチを回すにはウィンドウが短すぎる（あるいは `drainEstimate` が過大） | [§2](#2-drain-のキャリブレーション-drainestimate-と-terminationgraceperiod) |
| `RotationSpansNextWindow` 警告 | `RotationSpansNextWindow` Warning イベント | `t_rot_est + cooldownAfter` がウィンドウの閉じている区間を超え、1 件のローテーションが次の機会まで直列の start ゲートを握るため `K · C` が容量を過大評価する | [§2](#2-drain-のキャリブレーション-drainestimate-と-terminationgraceperiod) |
