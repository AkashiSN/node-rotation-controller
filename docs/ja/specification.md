...
advance(np, name):
  ...
  case draining:                             # 旧 NodeClaim の finalize 消滅待ち
      annotate(np, active-rotation-state=draining, draining-at=now if unset)   # 冪等な再表明（通常は no-op。np 側は
                                             #   cand.state より先に書かれる）。アンカー導入前に draining へ到達した
                                             #   ローテーション向けに draining-at をバックフィルする
...
| `pending` | placeholder が旧ノード以外の Ready なホスト上で Running、かつホストが同一 NodePool | `draining` | surge ターゲットを凍結（`do-not-disrupt` + `surge-for`）。NodePool に `active-rotation-state=draining` + `draining-at=now`（write-once、delete より先 — §4.2 の drain 所要時間アンカー）を書く。旧 NodeClaim を `delete` |
...
| `draining` | 旧 NodeClaim に `deletionTimestamp` が無い（リカバリ）| `draining` | 冪等な `delete` を再発行。`draining-at` が未設定ならバックフィル |
