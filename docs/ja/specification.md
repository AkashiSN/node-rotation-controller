...
ROTATION（論理シーケンス。各ステップは別々の reconcile）
  ...
  │     freeze(candidate.node, surge-for=candidate.name)    // surge-for + do-not-disrupt（所有権マーカー付き。
  │                                                       //   運用者が既に do-not-disrupt を付けている場合は除く）
  ...
  │                        + SOFT preferred nodeAffinity hostname NotIn {candidate.node, 自トリガを過ぎたノード},
  │                                                       // drain 対象ノード／期限間近のホストを避ける preference（§3.3, #96）。
  │                                                       //   soft なので Karpenter は新ノードを provision できる（RestrictedLabels）。
  │                                                       //   候補の hard 除外は cordon + surge_ready が担い、この項ではない。
  │                                                       //   どちらのリストも placeholder の（再）作成のたびに再計算
  │         requests     = sum(candidate 上の再スケジュール対象 Pod の requests), // DaemonSet / mirror / 完了済 / ノード固定は除外
  │         tolerations  = candidate.nodepool.spec.template.spec.taints,      // NodePool 自身の taint を許容し、workload が
  │                                                       //   使う tainted capacity に placeholder も乗せる（§3.3）。
  │                                                       //   startupTaints は除外（ノード Ready 後に除去される）
  ...
  │           annotate(nodepool, active-rotation-state=draining, draining-at=now)  // 永続的なフェーズ記録を先に
  │                                                     //   （完了時の outcome を決める）。draining-at（write-once）は
  │                                                     //   §4.2 の drain 所要時間のアンカー — 旧 NodeClaim の
  │                                                     //   deletionTimestamp は完了時には失われている
  ...
  │               emit_metrics(success, drain=now − draining-at)  // §4.2 の drain 所要時間（アンカーがある場合）
  │           else:                                     // pending から消滅（強制失効、§3.3）:
  │               emit_metrics(expired); alert          //   何も回転していない — cooldown も不要
  │           clear(nodepool, active-rotation, active-rotation-state, draining-at)  // 直列ゲートの解放は最後
...
> **ライフサイクルに関する注記。** `nodepool` 単位の系列は **NodePool が削除された時点で消去する** — コントローラが削除 reconcile で破棄する。ゲージは reconcile ごとに再計算されるため、削除されて reconcile が止まったプールはそのままだと最後の値を永遠にラッチしてしまう（消えたはずの `noderotation_drain_stuck = 1` が無限にアラートし続ける）。ラベル無しの `noderotation_window_active` はクラスタ全体のものなので影響を受けない。
...
|-----------|------------|--------|--------------|
| 非致命的 schedule 所見（§3.2 layer 1–2） | NodePool | 所見コード（例: `KBelowTwo`, `AVeryAggressive`, `TGPUnset`, `HardCapExceeded`, `RetryBackoffShort`, `ThroughputZero`, `ThroughputBelowArrival`, `OverrideGBelowK`） | その所見が NodePool で有効化されたとき |
...
│  │    - NodePool Reconciler; NodeClaim/Pod/Node を watch     ││
│  │    - 1 分の self-requeue（窓端・drain 進捗の検知）          ││
│  │    - /metrics                                             ││
│  │                                                           ││
...
| `karpenter.sh/do-not-disrupt` | 旧ノード + surge ターゲットノード | `true` | surge 中の Karpenter **voluntary disruption のみ**（Consolidation/Drift/Emptiness）をブロック — `expireAfter`・Interruption・Node Repair は**ブロックしない**（§3.3）。ノード上では、運用者が事前に設定した `do-not-disrupt` をコントローラが見つけなかった場合に限り付与し、その際 `noderotation.io/do-not-disrupt-owned` マーカーと対にする。最後に除去。残存値はノードの寿命を延ばさない（§3.5 参照）|
...
>
> **未検証（PoC スコープ）:** *primary* 機構 — 置換対象ノードの*再スケジュール対象* Pod requests（§3.3 の除外: DaemonSet / mirror / 完了済 / ノード固定）にサイズした placeholder Pod による NodePool-owned ノードの誘発、**voluntary** な Consolidation/Drift を退けるための surge 中の新旧両ノードへの `karpenter.sh/do-not-disrupt` 付与、そして明示的 NodeClaim 削除が旧ノードを voluntary（PDB 尊重）経路で drain すること、加えて preempt された bare placeholder Pod（専用の負値 `PriorityClass`・`preemptionPolicy: Never` で動く）が再 pending せず削除されること、surge 途中での無関係な高優先度 preempt が `readyTimeout` → ロールバックで有界であること（§3.3 *placeholder の優先度*）、required な `karpenter.sh/nodepool` セレクタが複数 NodePool のクラスタで kube-scheduler のバインドと Karpenter のプロビジョニングの両方を候補の NodePool に閉じ込めること（§3.3）、そして bin-pack された placeholder が drain のために既存キャパシティを実際に予約すること（capacity-absorb 経路、§3.3）の確認。これらが最初の PoC 項目。（`do-not-disrupt` が `expireAfter` をブロック**しない**ことは未解決の PoC 項目では *なく*、文書化された Karpenter の挙動であり `nodeclaim/expiration` コントローラのソースで確認済みである、§3.3。本設計は expiration レースに勝つために annotation ではなく `leadTime` サイジングに依拠する。）**更新（issue #96）:** 新規プロビジョニング誘発は当初 *negative* と判明した — placeholder 上の **required** な `kubernetes.io/hostname` 項は Karpenter のプロビジョナ（`RestrictedLabels`）に拒否され、capacity-absorb 経路しか完了しなかった。その除外を **soft preferred** 項に変換することで（§3.3）これが解消され、KWOK ハーネスは事前の spare 無しで新規 NodePool-owned NodeClaim を誘発して `complete` に到達する新規プロビジョニング surge を実証するようになった。残る実クラウド確認（同一 AZ キャパシティ、`limits` 枯渇、zonal-PV 再バインド、`expireAfter` soak レース）は引き続き EKS Auto Mode（#93）のスコープとする。**更新（issue #93、2026-06-22）:** これら実クラウド確認は上の検証済み行として記録された — placeholder Pod による make-before-break surge、voluntary 経路 drain、同一 AZ zonal-PV 再バインド、`readyTimeout` ロールバック + クリーンアップ、`limits` 枯渇ゲート、`expireAfter` バックストップ余裕 — いずれも ephemeral な EKS Auto Mode クラスタ（K8s 1.33、`karpenter.sh/v1`）上で実施。**更新（issue #93/#77/#81、2026-06-22、batch 2）:** 上の検証済み行に、複数 NodePool の **閉じ込め**、voluntary な **PDB 尊重 drain**、新旧両ノードの **`do-not-disrupt`**、強制失効の **`expired`** outcome、**スケール版 R6 soak**（3 回連続ローテーション）を追加。実クラウドで未了: bin-pack された **capacity-absorb** 経路（上記はすべて新規プロビジョニング経路）、再 pending せず preempt される bare placeholder、*無関係な高優先度 preempt* や本物の同一 AZ **キャパシティ不足（ICE）** によるロールバック誘発（いずれもオンデマンドで決定的に再現できないため短い `readyTimeout` で代替）、ウィンドウ境界の開始/停止と leader 交代の再開、そしてフルな数時間の *接戦* `expireAfter` soak（日次ウィンドウでは到達不能）。RBAC の十分性と `karpenter.sh/v1` CRD デコードは暗黙に検証済み — コントローラは surge オブジェクトの create/patch/delete と実 `NodeClaim` の reconcile をエラーなく実施。
