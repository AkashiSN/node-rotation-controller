...
以下の図は 1 回のローテーションの **論理** シーケンスである。単一のブロッキング呼び出しとしては **実行されない**。コントローラはこれを **非ブロッキングな requeue 駆動の状態機械**（§5.2）として実装し、進捗を旧 NodeClaim の `noderotation.io/state` annotation に保持し、ローテーション自体は NodePool の `noderotation.io/active-rotation` annotation にアンカーする — このアンカーは、成功時に削除される **旧 NodeClaim より長生き** し、完了ステップを駆動する（§5.3）。各 `wait_*` ステップは *後続の reconcile で再評価される状態* であって、worker をブロックする goroutine ではない。`[state: …]` タグが各ステップを state annotation の値に対応づける。
...
ROTATION（論理シーケンス。各ステップは別々の reconcile）
  ...
  │                        + candidate.node への anti-affinity,  // drain 対象ノード上には決して確保しない
  ...
  │           delete(placeholder)
  │           特定できれば surge NodeClaim を回収       // 明示的に。Consolidation 任せにしない（下のロールバック）
  ...
  │           clear(nodepool, active-rotation)          // 直列ゲートの解放は最後
...
| 失敗 | 動作 |
|------|------|
| 新ノードが timeout 内に `Ready` 化しない | placeholder を削除し、**誘発した surge NodeClaim も特定できれば明示的に削除する**（placeholder がバインド／nominate されている新ノード）。回収を Consolidation 任せに *しない*: consolidation が実質無効な NodePool（例: `WhenEmpty` + 長い `consolidateAfter`、窓外の `nodes: "0"` budgets）では、放棄された surge ノードが自身の `expireAfter` まで課金され続けるため。旧ノードの `do-not-disrupt` を除去、旧はそのまま、アラート発火 |
...
advance(np, name):
  cand := nodeclaim(name)
  if cand == nil:                            # 旧 NodeClaim が finalize 消滅 → ローテーション完了
      # （通常は draining から。pending から消えた場合 — 例: surge 中の force-expire、
      #   §3.3 残存リスク — も同じクリーンアップを適用）
      ...
      clear(np, active-rotation)             # 直列ゲートの解放は最後
      emit_metrics(success)
  ...
  case (none) | pending:                     # 冪等: このフェーズに必要なものを毎回（再）表明する
      ...
      annotate_once(cand, started-at=now)    # write-once: リトライでタイムアウトをリセットしない
      ...
      if placeholder(name) is missing:       # 喪失 / preempt / 作成前のクラッシュ
          ...
          return Requeue(30s)                #   DaemonSet/mirror/完了済/ノード固定）; cand.node への anti-affinity
      ...
      if elapsed(cand.started-at) > readyTimeout:        # 既定 15m
          ...
          reap_surge_claim(name)             # 明示回収。Consolidation 任せにしない（§3.3 ロールバック）
          unfreeze(cand.node)
  ...
  case draining:                             # 旧 NodeClaim の finalize 消滅待ち
      ...
      return Requeue(30s)
  case failed:                               # failed 書込とアンカー clear の間のクラッシュ
      clear(np, active-rotation)
      return Requeue(1m)
```

`pick_oldest_eligible` は `state` が空（新規）か、`failed` かつ `now − failed-at` がエスカレーション後の backoff（`retryBackoff · 2^(retry-count − 1)`、上限 8 倍）を超えた claim を選ぶ。`pending`/`draining` は新規候補として再選定されない（ステップ 1 が駆動）。Leader election は `coordination.k8s.io/Lease` 標準。リーダー交代時、新リーダーは annotation のみから再開する。

ステップ 1 のキーは **NodePool の `active-rotation` アンカー** であり、annotation 付き NodeClaim の発見ではない: ローテーションごとの `state` の担い手である旧 NodeClaim は成功時に削除されるため、それに依存する発見方法は、完了側の副作用（placeholder 除去・surge ターゲットの unfreeze・`last-rotation-at`）を実行すべきまさにその瞬間に盲目になる。アンカーは開始時に他のどの副作用 **よりも先に** 書き、完了/失敗時に **最後に** 消す — どのクラッシュ地点でも再開可能な記録が残る。これと相補的に、各状態ハンドラは一回限りのアクションではなくフェーズの望ましい状態の **冪等な再表明** である: pending ハンドラは旧ノードの freeze と placeholder の存在を毎パス再表明し（開始時の副作用のどの 2 つの間でクラッシュしても次の reconcile で治癒する）、draining ハンドラは旧 NodeClaim に `deletionTimestamp` が無ければ冪等な `delete` を再発行する（state 書込と delete の間のクラッシュは、これがなければ誰も要求していない削除を待ち続けてローテーションが永久にハングする）。
...
進行状態は Kubernetes オブジェクト（NodePool、旧 `NodeClaim`、新旧 2 ノード、一時的な placeholder Pod）にのみ持つ — **外部データストア不要**。永続的な真実は 2 つの担体に分かれる: NodePool の `active-rotation` アンカーが **どの** ローテーションが進行中かを記録し（成功時の旧 NodeClaim 削除を生き延びる）、旧 NodeClaim の `state` annotation がそのローテーションが **どこまで** 進んだかを記録する。placeholder Pod とノード上のマーカーはランタイムオブジェクトであり、失われてもこの 2 つから冪等なハンドラ（§5.2）が再作成・再表明する。
...
| `noderotation.io/started-at` | 旧 NodeClaim | RFC3339 | `readyTimeout` の期限 + observability（試行ごとに write-once）|
| `noderotation.io/failed-at` | 旧 NodeClaim | RFC3339 | 失敗後の再選定 backoff の基点 |
| `noderotation.io/retry-count` | 旧 NodeClaim | 整数 | この claim の連続失敗回数。backoff をエスカレーションさせ（`retryBackoff · 2^(retry-count − 1)`、上限 8 倍）、系統的失敗のアラートにも供する |
...
| From | イベント | To | 副作用 |
|------|---------|----|--------|
| *(なし)* | 窓内で選定 | `pending` | NodePool に `active-rotation` アンカーを書く（最初）。旧ノードを凍結（`do-not-disrupt` + `surge-for`）。placeholder 作成（旧ノードへの anti-affinity 付き）|
| `pending` | 各 reconcile（リカバリ）| `pending` | 旧ノードの凍結を再表明。placeholder 不在なら再作成 |
| `pending` | placeholder が旧ノード以外の Ready なホスト上で Running | `draining` | surge ターゲットを凍結（`do-not-disrupt` + `surge-for`）。旧 NodeClaim を `delete` |
| `pending` | `readyTimeout` 経過 | `failed` | placeholder 削除。誘発した surge NodeClaim を特定できれば明示回収（§3.3 *ロールバック*）。旧ノード unfreeze。`retry-count += 1`。アンカー clear。アラート |
...
| `draining` | 旧 NodeClaim 消滅 | *(完了)* | placeholder 削除。`surge-for` を持つノードを unfreeze。`last-rotation-at` 書込。アンカー clear（最後）。success 計上 |
| `failed` | エスカレーション後の backoff 経過かつ窓内 | `pending` | 新規開始として再入（`retry-count` は維持）。連続失敗は `expireAfter` backstop が担保 |
