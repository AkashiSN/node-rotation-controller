...
ROTATION（論理シーケンス。各ステップは別々の reconcile）
  ...
  │     //   preempt で失われた freeze や placeholder は次のパスで復元される
  │
  ...
  │           特定できれば surge NodeClaim を回収       // placeholder 削除より「先」 — そのバインド先こそが特定手段。
  │                                                     //   回収対象は started-at より後に作成された NodeClaim のみ —
  │                                                     //   既存の capacity-absorb ホストは決して回収しない（下のロールバック）
  │           delete(placeholder)
...
| 失敗 | 動作 |
|------|------|
| 新ノードが timeout 内に `Ready` 化しない | **誘発した surge NodeClaim を特定できれば明示的に削除し、その後に placeholder を削除する — この順序が必須**: placeholder のバインド／nominate 先こそが特定手段であり、placeholder を先に消すと失われる。回収対象になり得るのは **このローテーションの `started-at` より後に作成された NodeClaim のみ** — capacity-absorb 経路では placeholder は *既存* ノードにバインドされるが、それは健全な本番キャパシティであって surge の残骸ではなく、決して回収してはならない。回収を Consolidation 任せに *しない*: consolidation が実質無効な NodePool（例: `WhenEmpty` + 長い `consolidateAfter`、窓外の `nodes: "0"` budgets）では、放棄された surge ノードが自身の `expireAfter` まで課金され続けるため。旧ノードの `do-not-disrupt` を除去、旧はそのまま、アラート発火 |
...
advance(np, name):
  ...
  case (none) | pending:                     # 冪等: このフェーズに必要なものを毎回（再）表明する
      ...
      if elapsed(cand.started-at) > readyTimeout:        # 既定 15m。最初にチェックする: この失敗パスの途中で
          reap_surge_claim(name)             #   クラッシュしても、下の再作成分岐で placeholder を蘇生させないため。
                                             # 回収は placeholder 削除より「先」 — そのバインド先こそが特定手段。
                                             #   対象は started-at より後に作成された claim のみ —
                                             #   既存の absorb ホストは決して回収しない（§3.3 ロールバック）
          delete(placeholder(name))
          unfreeze(cand.node)
          annotate(cand, state=failed, failed-at=now, retry-count+=1); clear(cand, started-at)
          clear(np, active-rotation)         # ゲートを解放。failed claim は backoff 経由で再入する
...
| `noderotation.io/active-rotation-state` | NodePool | `draining` | アンカーされたローテーションのフェーズミラー。コントローラによる旧 NodeClaim の `delete` の直前に書く。不在ならローテーションは `pending` を出ていない。完了ハンドラが — `state` の担体である旧 NodeClaim の消滅後に — outcome を選ぶために読む: `draining` → `success` + cooldown、不在 → `expired` + アラート、cooldown なし（§5.2）。アンカーと同一 update で消去する（同一オブジェクト → アトミック）|
| `noderotation.io/state` | 旧 NodeClaim | `pending` / `draining` / `failed` | アンカーされたローテーションの進行ステート |
| `noderotation.io/started-at` | 旧 NodeClaim | RFC3339 | `readyTimeout` の期限 + observability。**試行ごとに** write-once: failed 書込で消去されるため、リトライは自分の期限を刻み直す（さもなければ `retryBackoff` ≥ `readyTimeout` のため、すべてのリトライが即時タイムアウトする）|
...
| `pending` | `readyTimeout` 経過 | `failed` | 誘発した surge NodeClaim を特定できれば明示回収 — `started-at` より後に作成されたもののみを、placeholder 削除 **より先に** 解決（§3.3 *ロールバック*）。placeholder 削除。旧ノード unfreeze。`retry-count += 1`。`started-at` 消去。アンカー clear。アラート |
...
| `failed` | エスカレーション後の backoff 経過かつ窓内 | `pending` | `advance()` の `failed` ケースが `state` を `pending` に戻す（§5.2）— `retry-count` は維持、`started-at` は新しい試行が刻み直す。連続失敗は `expireAfter` backstop が担保 |
