...
| From | イベント | To | 副作用 |
|------|---------|----|--------|
| *(なし)* | 窓内で選定 | `pending` | NodePool に `active-rotation` アンカーを書く（最初。conflict チェック付き only-if-absent — §5.2）。旧ノードを凍結（`do-not-disrupt` + `do-not-disrupt-owned` マーカー + `surge-for`。運用者が既に `do-not-disrupt` を設定している場合は do-not-disrupt 系を除く）。旧ノードを cordon（+ `cordoned` マーカー）。placeholder 作成（required な `karpenter.sh/nodepool` セレクタ。Pod レベルの `do-not-disrupt`。hostname `NotIn` で旧ノードと期限間近のホストを除外、§3.3）|
...
| `pending` | placeholder が旧ノード以外の Ready なホスト上で Running、かつホストが同一 NodePool | `draining` | surge ターゲットを凍結（`do-not-disrupt` + `do-not-disrupt-owned` マーカー + `surge-for`。運用者が既に `do-not-disrupt` を設定している場合は do-not-disrupt 系を除く）。NodePool に `active-rotation-state=draining` + `draining-at=now`（write-once、delete より先 — §4.2 の drain 所要時間アンカー）を書く。旧 NodeClaim を `delete` |
