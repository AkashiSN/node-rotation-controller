...
| `t_rot` | 1 ノードのローテーション所要上限 = `readyTimeout + tGP + cooldownAfter` | 設定 + NodePool から導出 |
...
**検証**（レイヤ2 — スループット） — 導出とは独立で、**警告のみ**・`A` は変えない。尺 `D` の各ウィンドウ機会で `C = m · floor(D / t_rot)` 台を捌ける（`m = surge.maxUnavailable`、v1 は `1` 固定）。候補到来率が容量を超える（`C < N · P / A`、`N` は NodePool 台数）と候補が滞留し一部が Forceful Expiration し得る:

- **warn**: ウィンドウ拡張（`D` 増）、機会追加（`P` 縮小）、または `maxUnavailable` 引き上げ（将来バージョン用）。

> **計算例。** Auto Mode, `E = 14d`, `tGP = 1h`, 和集合 `{Wed, Sat} 02:00–06:00` → `P = 4d`, `t_rot ≈ 1.5h`, `K = 2`。すると `A = 14d − (2·4d + 1.5h) ≈ 5.9d`: ノードは約 5.9d で候補化し、14d 前に 2 回の窓が保証される。スループット `C = floor(4h / 1.5h) = 2`/機会。
>
...
導出された `A`、保証回数 `G`、`P` は NodePool ごとに起動時ログとメトリクス（§4.2）で露出する。

## 3.3 surge シーケンス（v1）

1 reconcile で 1 ノードを処理。v1 は **直列固定（`surge.maxUnavailable = 1`）** で blast radius を最小化。
...
代わりに、対象 NodePool のラベルに合致する `nodeSelector`/affinity と、既存ノードに bin-pack できない大きさの resource requests を持つ一時的な **placeholder ワークロード**（低優先度の "pause" Pod）を作成し、Karpenter にその NodePool 内へ新ノードを誘発させる。旧ノードの drain 後に placeholder を削除し、新ノードは NodePool の通常メンバーとして残る。
...
両方を防ぐため、surge の間 **新旧両ノード** に `karpenter.sh/do-not-disrupt` を付与する。これは Karpenter の *voluntary* disruption のみをブロックする。コントローラ自身の明示的な旧 NodeClaim `delete` は drain を進める（削除は termination controller が処理し、`do-not-disrupt` を参照しない）。annotation は最後に除去し、新ノードを通常管理へ戻す。
...
ROTATION（論理シーケンス。各ステップは別々の reconcile）
  ...
  │
  ├─ surge_ready?（placeholder がスケジュール → そのノードが Ready）   [state: pending]
...
> placeholder の唯一の役割は、drain の前に NodePool へちょうど 1 ノード分の capacity を先出しすること（make-before-break）。*新* ノードを確実に誘発する（既存ノードに bin-pack させない）ための requests サイズは環境依存で、PoC で確定する。
...
| 並列 surge 数 | v1 は `surge.maxUnavailable = 1`（直列）固定。代替ノードは placeholder ワークロード経由で誘発される **NodePool-owned** ノード（§3.3）のため、NodePool の `limits`（および外部の EC2 vCPU クォータ）はベースライン +1 ノードを許容する必要がある。`maxUnavailable > 1` は将来バージョン用の予約で、その場合 +m の余裕が要る |
...
advance(cand):
  ...
  case pending:                              # surge ノードの Ready 待ち
      if surge_ready(cand):                  # placeholder がスケジュール → そのノードが Ready
...
| `karpenter.sh/do-not-disrupt` | 旧ノード + 新ノード | `true` | surge 中の Karpenter voluntary disruption をブロック（最後に除去）|
...
data:
  policy.yaml: |
    ...
    surge:
      ...
      cooldownAfter: 10m       # 成功後、次のローテーションまでの休止
...
>
> **未検証（PoC スコープ）:** *primary* 機構 — placeholder ワークロードによる NodePool-owned ノードの誘発、surge 中の新旧両ノードへの `karpenter.sh/do-not-disrupt` 付与、そして Auto Mode マネージド Karpenter が `do-not-disrupt` を尊重しつつ明示的 NodeClaim 削除では drain が進むことの確認。これらが最初の PoC 項目。
