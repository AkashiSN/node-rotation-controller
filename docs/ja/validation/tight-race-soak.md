...
|:---|:---|:---|:---|
| 12h 本編の graceful surge 回転（5.0 回/h） | `expired`・`failure`・本編 fallback・`short_lead`・restart | deadline margin 最小値（中央値 70.3 / 最大 71.2） | エピローグ解凍 → surge-less fallback 完了 |
...
T0 から 12 時間で本編プールは **71 回転、すべて新規 provision ノードへの
graceful make-before-break surge** を完了した。完了は約 12 分間隔で刻まれ、
5 スロットが位相を保ったまま回り続けた。margin（= 旧 claim の deadline −
回転完了時刻）は 71 回転すべてで 68.3〜71.2 分に収まり、分散は 3 分未満 —
**12 時間かけても劣化の蓄積が皆無**だったことを示す。
...
| 2 | `success` が ≈5/h で単調増加、計 ≥ 40 | PASS | 71（epi 込み 72）、~12m 間隔 |
...
| 証拠 | `forceful_fallback_total{nodepool-soak-epi}` 0→1 ・ spec 文言どおりの `ForcefulFallback` Warning イベント ・ placeholder 台帳（`noderotation.io/surge-for` の `pods -w` 連続監視）に epi claim の記録**ゼロ** ・ `expired` 終始 0 ・ 本編プールはエピローグ中も無擾乱（70→71、完了間隔 ≤ `P + t_rot` = 42m） |
