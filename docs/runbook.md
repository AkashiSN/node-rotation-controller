...
| `NodeRotationWindowMissed` | A maintenance window closed with candidates unrotated | See [below](#responding-to-a-lost-window-noderotationwindowmissed) |
...
- Attempts inside one maintenance window are paced by `readyTimeout + failurePause`, not by `retryBackoff`: since the window-aware clamp (spec §3.2) a failed claim's retry is held inside the occurrence it failed in, so `retryBackoff` no longer ends the window early, and it does not bound the pool-wide attempt rate either — it is per claim, and several claims alternate independently. For a window of duration `D` the order-of-magnitude ceiling on timeout-driven attempts is `1 + D / (readyTimeout + failurePause)`. If that is more churn than you want, raise `surge.failurePause`; raising `retryBackoff` alone will not do it.
