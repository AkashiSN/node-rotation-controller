...
| NodePool never rotates, no surge is ever created | `InsufficientHeadroom` Warning Event on the NodePool | The pool's `spec.limits` leave no room for the placeholder, which reserves replacement capacity *before* draining. The Event names the resource, what the placeholder needs, and what remains of the ceiling: raise `spec.limits` by at least that shortfall, or reduce the pool's provisioned capacity. If `surge.wholeNodeReservation` is on, the gate tests a whole node's footprint (§3.3) — the pool needs a whole instance of headroom, not a drain's worth |
