...
const until = (e: SimEvent) => new Date(e.until ?? e.at).getTime()
...
const bars = computed(() => rows.value.map((name, i) => {
  ...
  const born = declared ? new Date(declared.createdAt).getTime() : bornEvent ? at(bornEvent) : t0.value
  const done = props.events.find(e => e.kind === 'rotation-done' && e.node === name)
  const eff = declared?.expireAfter ?? props.fleet.expireAfter
  const deadline = born + (parseGoDuration(eff) ?? 0)
  return {
    name,
    y: PAD_T + i * ROW,
    x1: x(clip(born)),
    x2: x(clip(done ? at(done) : t1.value)),
    deadline: deadline <= t1.value && deadline >= t0.value ? x(deadline) : null,
  }
}))
...
<template>
  <section class="sim-block">
    <h3>{{ t.timeline }}</h3>
    <svg class="sim-svg" :viewBox="`0 0 ${W} ${height}`" role="img">
      <!-- maintenance windows -->
      ...
      <g v-for="b in bars" :key="b.name">
        ...
        <line :x1="b.x1" :y1="b.y - 4" :x2="b.x2" :y2="b.y - 4" class="sim-life" />
