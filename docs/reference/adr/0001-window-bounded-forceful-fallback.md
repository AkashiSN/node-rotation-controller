...
>
> **Note (2026-07-11, [#220](https://github.com/AkashiSN/node-rotation-controller/issues/220)).** The `t_rot_est = readyTimeout + surge.drainEstimate + buffer` formula in the #212 note above is itself superseded by [ADR-0003](0003-provisioning-estimate-vs-surge-abandon-deadline.md): after #220 the forecast is `t_rot_est = provisioningEstimate + drainEstimate`, with `buffer` and the `readyTimeout`/`tGP` deadline terms deadline-side only (they stay in `t_rot`). The forceful-fallback deadline race here still uses `t_rot`, unchanged, and the decision is still unaffected — it turns on `C·A < N·P`, not on which terms feed `C`.
