...
>
> **Note (2026-07-11, [#220](https://github.com/AkashiSN/node-rotation-controller/issues/220)).** The `t_rot_est = readyTimeout + drainEstimate + buffer` formula below is superseded by [ADR-0003](0003-provisioning-estimate-vs-surge-abandon-deadline.md), which drops `buffer` and replaces `readyTimeout` with a `provisioningEstimate` term for the same reason this ADR replaced `tGP` with `drainEstimate`: `readyTimeout` is the surge-abandon *deadline*, not the expected provisioning duration. After #220 `t_rot_est = provisioningEstimate + drainEstimate`, so on the §3.2 worked schedule `C` goes `1 → 10` and `K·C` `2 → 20` (the *Consequences* `1 → 5` / `2 → 10` figures — already re-stated `1 → 7` / `2 → 14` by the #215 note above — are likewise superseded). The **deadline/forecast split, the containment boundary, and the "never fit a deadline to observation" argument are unaffected**; `drainEstimate` itself is unchanged.
