...
>
> **Not yet validated (PoC scope):** the *primary* mechanism — inducing a NodePool-owned node via a placeholder Pod sized to the candidate node's *reschedulable* Pod requests (§3.3 exclusions: DaemonSet / mirror / completed / node-pinned), applying `karpenter.sh/do-not-disrupt` to both nodes during the surge to fend off **voluntary** Consolidation/Drift, and confirming that explicit NodeClaim deletion drains the old node through the voluntary (PDB-respecting) path, plus that a preempted bare placeholder Pod (running under its dedicated negative `PriorityClass`, `preemptionPolicy: Never`) is deleted without re-pending, that an unrelated higher-priority preemption mid-surge is bounded by `readyTimeout` → rollback (§3.3 *Placeholder priority*), that the required `karpenter.sh/nodepool` selector confines both kube-scheduler binding and Karpenter provisioning to the candidate's NodePool in a multi-NodePool cluster (§3.3), and that a bin-packed placeholder genuinely reserves existing capacity for the drain (capacity-absorb path, §3.3). These are the first PoC items. (That `do-not-disrupt` does **not** block `expireAfter` is *not* an open PoC question — it is documented Karpenter behavior, confirmed in the `nodeclaim/expiration` controller source; §3.3. The design relies on `leadTime` sizing, not the annotation, to win the expiration race.) **Update (issue #96):** the new-provision inducement was first found *negative* — a **required** `kubernetes.io/hostname` term on the placeholder is rejected by Karpenter's provisioner (`RestrictedLabels`), so only the capacity-absorb path completed. Converting that exclusion to a **soft preferred** term (§3.3) unblocks it; the KWOK harness now demonstrates a new-provision surge reaching `complete` by inducing a brand-new NodePool-owned NodeClaim with **no** pre-existing spare. The remaining real-cloud confirmations (same-AZ capacity, `limits` exhaustion, zonal-PV rebind, the `expireAfter` soak race) stay scoped to EKS Auto Mode (#93). **Update (issue #93, 2026-06-22):** these real-cloud confirmations are now recorded as the Validated rows above — the placeholder-Pod make-before-break surge, voluntary-path drain, same-AZ zonal-PV rebind, `readyTimeout` rollback + cleanup, `limits`-exhaustion gating, and the `expireAfter` backstop margin — all on an ephemeral EKS Auto Mode cluster (K8s 1.33, `karpenter.sh/v1`). **Update (issue #93/#77/#81, 2026-06-22, batch 2):** further Validated rows above add multi-NodePool **confinement**, the voluntary **PDB-respecting drain**, **`do-not-disrupt`** on both surge nodes, the **`expired`** force-expiry outcome, and a **scaled R6 soak** (3 consecutive rotations). Still open on real cloud: the bin-packed **capacity-absorb** path (the runs above all took the new-provision path); a bare placeholder preempted without re-pending; an *unrelated higher-priority preemption* or a genuine same-AZ **capacity shortage (ICE)** driving the rollback (both were stood in for by a short `readyTimeout`, since neither is deterministically inducible on demand); window-boundary start/stop and leader-change resume; and a full multi-hour *tight-race* `expireAfter` soak (not achievable under a daily window). RBAC sufficiency and `karpenter.sh/v1` CRD decode are implicitly exercised — the controller creates/patches/deletes the surge objects and reconciles real `NodeClaim`s without error.
