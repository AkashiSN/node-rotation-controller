...
| `expireAfter` stays a **backstop** the controller's lead time wins against (R6) | **Validated** (by construction + observed) — EKS Auto Mode (2026-06-22) | The candidate retained `expireAfter=336h` while the controller rotated at `ageThreshold=5m` with `noderotation_rotation_chances=13` and `noderotation_short_lead_nodes=0`; observed rotations all fired at ~5m, far inside the backstop. A full multi-hour soak is deferred |
