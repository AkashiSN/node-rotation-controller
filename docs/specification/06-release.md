...
| v1.0 | Stable `RotationPolicy` CRD (`v1`), documented production runbook, soak-tested on a real EKS Auto Mode cluster. One item from §7.2 remains open: a genuine same-AZ capacity shortage (ICE) driving rollback. The full multi-hour tight-race `expireAfter` soak (§7.2, issue #118) is now validated |
