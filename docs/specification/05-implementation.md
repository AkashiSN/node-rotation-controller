...
- **every claim-state write claims its transition the same way**, and accepts only the states its handler is dispatched on — the two writes into `expired`, `pending`'s entry assertion, and the `failed` → `pending` retry. A pass arriving on a cached claim already written terminal cleans up, releases the anchor and emits nothing; a pass whose claim has moved on to any other state writes nothing at all, touches none of the rotation's runtime objects, and leaves it to the handler that owns it
