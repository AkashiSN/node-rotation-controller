...
- the reconcile's remaining claim-state writes stay unconditional, and are safe structurally rather than by veto: `pending` → `draining`, the forceful fallback's `draining`, and `pending` → `failed` are each made by the pass that has just done the work they record, inside a handler that already claimed its own entry, and each moves the claim forward. The §5.3 startup sweep's write sits outside this dispatch altogether
