...
- **whole-node reservation:** Opt-in mode (`surge.wholeNodeReservation`, default off; ADR-0005) that sizes the placeholder to a whole node's worth of what the drain needs, so a host already running other Pods cannot absorb it (§3.3)
