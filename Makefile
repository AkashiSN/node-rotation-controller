...
# kubectl, terraform) are pinned in aqua.yaml and resolved from $PATH via aqua —
# install aqua and put `$(aqua root-dir)/bin` on $PATH (see aqua.yaml header).
# They are invoked here by bare name; aqua lazily installs the pinned version on
# first use. LOCALBIN is only the build/output dir for the manager binary and
# the envtest control-plane assets.
