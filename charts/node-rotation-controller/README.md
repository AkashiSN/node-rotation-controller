...
| `PrometheusRule` | The eight §4.2 alerts | `prometheusRule.enabled` |
...
The `PrometheusRule` ships eight alerts. Several depend on your window cadence —
...
| `prometheusRule.enabled` | `false` | Create a PrometheusRule with the eight §4.2 alerts. Requires the `monitoring.coreos.com/v1` CRD. |
| `prometheusRule.labels` | `{}` | Extra labels for the PrometheusRule. |
| `prometheusRule.<alert>.for` / `.severity` | per alert | Per-alert `for` duration and severity. Alerts: `completedFailure`, `candidatesNotDraining`, `stalledInWindow`, `drainStuck`, `shortLeadNodes`, `retryCountHigh`, `forcefulFallback` (ships `severity: info` — a single fallback is by design per ADR-0001; tighten it for your environment), `windowMissed`. |
...
| `prometheusRule.windowMissed.range` | `24h` | `increase()` range the lost-window counter is checked over; set to comfortably span your window period (issue #303). |
