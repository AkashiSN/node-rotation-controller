...
| R6 | `expireAfter` stays a backstop the controller's lead time wins against | validated via a **scaled** multi-rotation soak; the full multi-hour tight-race soak is covered by Scenario P (see SCENARIOS.md) — validated 2026-07-15, 71/71 graceful rotations, `expired`/main-pool `forceful_fallback` both 0 |
