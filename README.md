...
[![Status](https://img.shields.io/badge/status-v0.7.0-rc.1_released_(pre--1.0)-blue.svg)](docs/specification/)
...
**v0.7.0-rc.1 — latest release; the v1 surge MVP, pre-1.0.** This section describes `main`, which may be ahead of that release; what changed in each release, and what to do on upgrade, is in the [changelog](CHANGELOG.md).
...
Two behaviors are **opt-in and default off**:

- **Window-bounded forceful fallback** (`surge.forcefulFallback`; §3.6, ADR-0001) deletes a candidate's `NodeClaim` inside the window — still via Karpenter's voluntary, PDB-respecting path — when a graceful surge cannot finish before that node's own `expireAfter` deadline, rather than leaving it to be force-expired at an uncontrolled time.
- **Whole-node surge reservation** (`surge.wholeNodeReservation`; §3.3, ADR-0005) sizes the placeholder to a whole node's worth of cpu and memory, so a host whose free capacity is short of a node's cannot absorb it.
